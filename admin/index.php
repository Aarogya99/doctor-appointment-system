...
<body>
    ...
    $sql = "SELECT DATE_FORMAT(appodate, '%Y-%m') as month, COUNT(*) as total 
        FROM appointment GROUP BY month ORDER BY month";
    ...
    file_put_contents("monthly_appointments.csv", $data);
...
<tr>
    <td colspan="4">
        <div class="dashboard-items" style="padding: 20px; margin: 20px; width: 90%; display: block;">
            <h3 style="text-align:left; margin-bottom: 10px;">Predicted Appointments (Next 3 Months)</h3>
            <table class="sub-table" style="width: 100%; border-collapse: collapse; text-align: center;">
                <thead>
                    <tr style="background-color: #f2f2f2;">
                        <th style="padding: 10px;">Future Month</th>
                        <th style="padding: 10px;">Predicted Appointments</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    exec("python3 ../ml/forecast_arima.py");
                    $forecast = array_map('str_getcsv', file('forecast.csv'));
                    foreach ($forecast as $index => $row) {
                        if ($index == 0) continue;
                        echo "<tr>";
                        echo "<td style='padding: 10px;'>Month $index</td>";
                        echo "<td style='padding: 10px;'>" . number_format($row[0], 2) . "</td>";
                        echo "</tr>";
                    }
                    ?>
                </tbody>
