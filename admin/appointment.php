...
<body>
    ...
    <td colspan="4" style="padding-top: 20px;">
        ...
        $sql = "SELECT appointment.appoid, appointment.apponum, appointment.status, 
                schedule.title, schedule.scheduledate, schedule.scheduletime,
                patient.pname, doctor.docname
                FROM appointment
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (!empty($_POST['sheduledate'])) {
                $date = $database->real_escape_string($_POST['sheduledate']);
        ...
        $sql .= " ORDER BY appointment.appodate DESC";
            ...
            <tr>
                ...
                <td style="display: flex; gap: 8px; align-items: center;">
                    <?php if ($row['status'] == 'pending'): ?>
                        <a href="approve_appointment.php?id=<?= $row['appoid'] ?>" class="btn-primary btn">Approve</a>
                    <?php else: ?>
                        <span class="btn-secondary btn disabled">Approved</span>
                    <?php endif; ?>
                    <a href="?action=drop&id=<?= $row['appoid'] ?>&name=<?= urlencode($row['pname']) ?>&session=<?= urlencode($row['title']) ?>&apponum=<?= urlencode($row['apponum']) ?>" 
